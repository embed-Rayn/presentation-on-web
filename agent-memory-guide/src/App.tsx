...
function App() {
  // 로컬 스토리지에 저장된 스피커 노트 정보 로드
  const [slides, setSlides] = useState<SlideData[]>(() => {
    const saved = localStorage.getItem('presentation_slides_notes');
    if (saved) {
      try {
        const notesMap = JSON.parse(saved);
        return DEMO_SLIDES.map(slide => ({
          ...slide,
          notes: notesMap[slide.id] !== undefined ? notesMap[slide.id] : slide.notes
        }));
      } catch (e) {
    ...
    const handleSyncMessage = (event: MessageEvent) => {
      ...
      switch (type) {
        ...
        case 'TRANSITION_CHANGE':
          ...
          break;
        case 'UPDATE_NOTES': {
          const { slideId, notes } = payload;
          setSlides(prev => prev.map(s => s.id === slideId ? { ...s, notes } : s));
          break;
        }
        default:
          break;
  ...
  // 스피커 노트 변경 전파 및 저장 함수
  const handleNotesChange = (slideId: string, newNotes: string) => {
    setSlides(prev => {
      const updated = prev.map(s => s.id === slideId ? { ...s, notes: newNotes } : s);
      // localStorage에 스피커 노트 정보 저장
      ...
      localStorage.setItem('presentation_slides_notes', JSON.stringify(notesMap));
      return updated;
    });

    // BroadcastChannel을 통해 다른 창에 스피커 노트 변경 동기화 전파
    syncChannel.postMessage({ type: 'UPDATE_NOTES', payload: { slideId, notes: newNotes } });
  };
