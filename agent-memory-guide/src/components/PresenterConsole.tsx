...
export const PresenterConsole: React.FC<PresenterConsoleProps> = ({
  ...
  const [notesText, setNotesText] = useState(currentSlide.notes || '');
  const [saveStatus, setSaveStatus] = useState('');

  // 슬라이드 이동 시 노트 로딩 및 저장 상태 리셋
  useEffect(() => {
    setNotesText(currentSlide.notes || '');
    setSaveStatus('');
  }, [currentSlideIndex, currentSlide.notes]);

  const handleSave = () => {
    if (onNotesChange) {
      onNotesChange(currentSlide.id, notesText);
      setSaveStatus('저장됨');
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: '#9ca3af', fontWeight: 600, fontSize: '0.9rem', width: '100%' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <BookOpen size={16} />
            <span>SPEAKER NOTES</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            {saveStatus && <span style={{ fontSize: '0.8rem', color: '#10b981', fontWeight: 500 }}>{saveStatus}</span>}
            <button
              onClick={handleSave}
              style={{
                background: 'var(--accent)',
